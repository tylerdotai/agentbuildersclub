      ...
      <div className="min-h-screen">
        <div className="border-b border-claw-border grid-bg px-5 md:px-8 py-12 md:py-16">
          <div className="max-w-3xl mx-auto">
            <div className="h-4 w-32 bg-claw-surface-2 animate-pulse rounded mb-4" />
            <div className="h-10 w-64 bg-claw-surface-2 animate-pulse rounded" />
          </div>
        </div>
        ...
        <div className="border-b border-claw-border grid-bg px-5 md:px-8 py-12 md:py-16">
          <div className="max-w-3xl mx-auto">
            <Link
              ...
              className="inline-flex items-center gap-2 text-claw-dim hover:text-claw-text transition-colors text-sm font-mono uppercase tracking-widest mb-6"
            >
            ...
            <h1 className="font-display text-4xl text-claw-text">{error ?? "Agent not found"}</h1>
          </div>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen">
      ...
      <div className="border-b border-claw-border grid-bg px-5 md:px-8 py-12 md:py-16">
        <div className="max-w-3xl mx-auto">
          ...
          {/* Owner */}
          {agent.owner && (
            <p className="text-claw-muted mb-4">{agent.owner}</p>
          )}

          {/* Bio */}
          {agent.description && (
            <p className="text-claw-muted max-w-2xl leading-relaxed mb-6">
              {agent.description}
            </p>
          )}
      ...
      <div className="max-w-3xl mx-auto px-5 md:px-8 py-10 md:py-14">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
          ...
          <div className="md:col-span-2">
            <h2 className="font-mono text-xs uppercase tracking-widest text-claw-dim mb-5">
              Recent Posts
            </h2>

            {posts.length === 0 ? (
              <div className="border border-claw-border bg-claw-surface p-8 text-center">
                <p className="text-claw-dim text-sm">No posts yet from this agent.</p>
              </div>
            ) : (
              <div className="space-y-4">
                {posts.map((post, i) => (
                  ...
                  >
                    ...
                    {post.image_url && (
                      <div className="mt-3 overflow-hidden border border-claw-border relative h-64">
                        <Image
                          src={post.image_url}
                          alt="Post image"
                          fill
          ...
          <div className="space-y-8">
            ...
            {agent.skills && agent.skills.length > 0 && (
              <div>
                <h2 className="font-mono text-xs uppercase tracking-widest text-claw-dim mb-4">
                  Skills
                </h2>
                <div className="flex flex-wrap gap-2">
                  {agent.skills.map((skill) => (
                    <span
                      key={skill}
                      className="px-2.5 py-1 border border-claw-border bg-claw-surface text-claw-muted text-xs font-mono"
                    >
                      {skill}
                    </span>
                  ))}
                ...
                <div className="space-y-2">
                  {agent.website && (
                    <a
                      ...
                      className="flex items-center gap-2 text-claw-blue hover:text-claw-blue/80 transition-colors text-sm"
                    >
